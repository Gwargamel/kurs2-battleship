...
const rows = 10; //anger spelbrädet
const cols = 10; //anger spelbrädet
const nrOfShips = 5; //anger antal skepp

const player1 = {
  mark: 1, //vilket tecken som ska markeras på brädet
  boms: [], //coordinates för missade skott, [{row: nr, col: nr}]
  hits: [], //coordinates för träffar, [{row: nr, col: nr}]
  ships: [], //coordinates för skepp, [{row: nr, col: nr}]
};

const player2 = {
  mark: 2, //vilket tecken som ska markeras på brädet
  boms: [], //coordinates för missade skott, [{row: nr, col: nr}]
  hits: [], //coordinates för träffar, [{row: nr, col: nr}]
  ships: [], //coordinates för skepp, [{row: nr, col: nr}]
};
...
function isValidShip(ship) {
  ...
  for (let i = 0; i < ship.length; i++) {
    if (ship.length > 5 || ship.length < 2) {
      return false
    } else if (!isValidCoord(ship[i])) {
      return false
    } else if (!ship.every(isValidCoord)) {
      return false
    } else {
      const firstCoord = ship[0];
      const row = firstCoord.row;
      const col = firstCoord.col;
      const isRowStraight = ship.every(function(coord) {
        return coord.row === row;
      });
      const isColStraight = ship.every(function(coord) {
        return coord.col === col;
      });
      return isRowStraight || isColStraight;
    }
  }
}
...
function hasLost(player) {
  /* TODO */
// kolla om player.hits är samma som player.ships 
...
function registerHitOrBom(guess, player) {
  /* TODO */
  //loopa genom players.ships, if ships.row || ships.col === gegisterHitOrBom pusha till hits[]
//else if if ships.row || ships.col !== gegisterHitOrBom pusha till boms[]
let previousHitLength = player.hits.length
...
if (player.hits.length === previousHitLength)
